...
impl SeatState {
	...
	{
		let inner = self.seats.iter().find(|inner| &inner.seat == seat).ok_or(SeatError::DeadObject)?;
...
pub trait KeyboardHandler: SeatHandler + Sized {
	...
	fn press_key(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, keyboard: &wl_keyboard::WlKeyboard, serial: u32, event: KeyEvent);

	/// A key has been released.
	///
	/// This stops the key from being repeated if the key is the last key which was pressed.
	fn release_key(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, keyboard: &wl_keyboard::WlKeyboard, serial: u32, event: KeyEvent);
	...
	/// used.
	fn update_repeat_info(&mut self, _conn: &mut ConnectionHandle, _qh: &QueueHandle<Self>, _keyboard: &wl_keyboard::WlKeyboard, _info: RepeatInfo) {}
}
...
impl KeyboardData {
	fn init_compose(&self) {
			...
			.to_str()
		{
			...
			if let Ok(table) = xkb::compose::Table::new_from_locale(&xkb_context, locale, xkb::compose::COMPILE_NO_FLAGS) {
				let compose_state = xkb::compose::State::new(&table, xkb::compose::COMPILE_NO_FLAGS);
	...
	D: Dispatch<wl_keyboard::WlKeyboard, UserData = Self::UserData> + KeyboardHandler,
{
	fn event(
		...
		if keyboard.version() < 4 && udata.first_event.load(Ordering::SeqCst) {
			...
			data.update_repeat_info(conn, qh, keyboard, RepeatInfo::Repeat { rate: NonZeroU32::new(200).unwrap(), delay: 200 });
		}
		...
		match event {
				...
				match format {
					WEnum::Value(format) => match format {
						...
						wl_keyboard::KeymapFormat::XkbV1 => {
							...
							match unsafe { xkb::Keymap::new_from_fd(&context, fd, size as usize, xkb::KEYMAP_FORMAT_TEXT_V1, xkb::COMPILE_NO_FLAGS) }
							{
								Some(keymap) => {
			...
			wl_keyboard::Event::Enter { serial, surface, keys } => {
				...
				if let Some(guard) = state_guard.as_ref() {
					...
					let raw = keys.chunks_exact(4).flat_map(TryInto::<[u8; 4]>::try_into).map(u32::from_le_bytes).collect::<Vec<_>>();
			...
			wl_keyboard::Event::Modifiers { serial, mods_depressed, mods_latched, mods_locked, group } => {
				...
				let mask = match guard.as_mut() {
					Some(state) => state.update_mask(mods_depressed, mods_latched, mods_locked, 0, 0, group),
					None => return,
				};
			...
			wl_keyboard::Event::RepeatInfo { rate, delay } => {
				let info = if rate != 0 {
					RepeatInfo::Repeat { rate: NonZeroU32::new(rate as u32).unwrap(), delay: delay as u32 }
				} else {
					RepeatInfo::Disable
				};
