...
{
	fn event(data: &mut D, touch: &WlTouch, event: TouchEvent, udata: &Self::UserData, conn: &mut ConnectionHandle, qh: &QueueHandle<D>) {
		match event {
...
fn process_framed_event<D>(data: &mut D, touch: &WlTouch, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, event: TouchEvent)
where
