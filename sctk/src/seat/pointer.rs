...
pub trait PointerHandler: SeatHandler + Sized {
	...
	fn pointer_press_button(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, pointer: &wl_pointer::WlPointer, time: u32, button: u32);
			...
			*/
			...
			wl_pointer::Event::Button { time, button, state, .. } => match state {
				WEnum::Value(state) => {
					if pointer.version() < 5 {
						match state {
							wl_pointer::ButtonState::Released => data.pointer_release_button(conn, qh, pointer, time, button),
							wl_pointer::ButtonState::Pressed => data.pointer_press_button(conn, qh, pointer, time, button),
				...
				match axis {
					WEnum::Value(axis) => {
						let (horizontal, vertical) = match axis {
							wl_pointer::Axis::VerticalScroll => (None, Some(AxisKind::Absolute(value))),
							wl_pointer::Axis::HorizontalScroll => (Some(AxisKind::Absolute(value)), None),
						...
						if pointer.version() < 5 {
							...
							let axis = guard.axis.get_or_insert(Axis { horizontal, vertical, source: None, time: Some(time) });
			...
			wl_pointer::Event::AxisSource { axis_source } => match axis_source {
				WEnum::Value(source) => {
					...
					let axis = guard.axis.get_or_insert(Axis { horizontal: None, vertical: None, source: Some(source), time: None });
			...
			wl_pointer::Event::AxisDiscrete { axis, discrete } => match axis {
				WEnum::Value(axis) => {
					...
					let (horizontal, vertical) = match axis {
						wl_pointer::Axis::VerticalScroll => (None, Some(AxisKind::Discrete(discrete))),

						wl_pointer::Axis::HorizontalScroll => (Some(AxisKind::Discrete(discrete)), None),

						_ => unreachable!(),
					};

					let axis = guard.axis.get_or_insert(Axis { horizontal, vertical, source: None, time: None });
			...
			wl_pointer::Event::Frame => {
				...
				if let Some(axis) = axis {
					let scroll = PointerScroll { horizontal: axis.horizontal, vertical: axis.vertical, source: axis.source };
				...
				if let Some(button) = button {
					match button.state {
						wl_pointer::ButtonState::Released => {
							data.pointer_release_button(conn, qh, pointer, button.time, button.button);
						}

						wl_pointer::ButtonState::Pressed => {
							data.pointer_press_button(conn, qh, pointer, button.time, button.button);
						}
