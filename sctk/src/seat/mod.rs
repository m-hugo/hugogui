...
impl SeatState {
	...
	pub fn get_touch<D>(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, seat: &wl_seat::WlSeat) -> Result<wl_touch::WlTouch, SeatError>
	where
...
pub trait SeatHandler: Sized {
	...
	fn new_seat(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, seat: wl_seat::WlSeat);
	...
	fn remove_capability(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, seat: wl_seat::WlSeat, capability: Capability);

	/// A seat has been removed.
	///
	/// The seat is destroyed and all capability objects created from it are invalid.
	fn remove_seat(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, seat: wl_seat::WlSeat);
}
...
{
	fn event(state: &mut D, seat: &wl_seat::WlSeat, event: wl_seat::Event, data: &Self::UserData, conn: &mut ConnectionHandle, qh: &QueueHandle<D>) {
		match event {
			wl_seat::Event::Capabilities { capabilities } => {
				...
				if keyboard != has_keyboard {
					...
					match keyboard {
						...
						false => state.remove_capability(conn, qh, seat.clone(), Capability::Keyboard),
					}
				}
				...
				if pointer != has_pointer {
					...
					match pointer {
						...
						false => state.remove_capability(conn, qh, seat.clone(), Capability::Pointer),
					}
				}
	...
	D: Dispatch<wl_seat::WlSeat, UserData = SeatData> + SeatHandler + ProvidesRegistryState + 'static,
{
	fn new_global(state: &mut D, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, interface: &str, version: u32) {
		if interface == "wl_seat" {
			...
			state.seat_state().seats.push(SeatInner { global_name: name, seat: seat.clone(), data });
