...
pub trait OutputHandler: Sized {
	...
	fn new_output(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, output: wl_output::WlOutput);

	/// An existing output has changed.
	fn update_output(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, output: wl_output::WlOutput);

	/// An output is no longer advertised.
	///
	/// The info passed to this function was the state of the output before destruction.
	fn output_destroyed(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, output: wl_output::WlOutput);
}
...
impl OutputState {
	...
	pub fn info(&self, output: &wl_output::WlOutput) -> Option<OutputInfo> {
		self.outputs.iter().find(|inner| &inner.wl_output == output).and_then(|inner| inner.current_info.clone())
	}
}
	...
	fn event(
		...
		match event {
			wl_output::Event::Geometry { x, y, physical_width, physical_height, subpixel, make, model, transform } => {
				let inner = state.output_state().outputs.iter_mut().find(|inner| &inner.wl_output == output).expect("Received event for dead output");
			...
			wl_output::Event::Mode { flags, width, height, refresh } => {
				...
				if let Some((index, _)) =
					inner.pending_info.modes.iter().enumerate().find(|(_, mode)| mode.dimensions == (width, height) && mode.refresh_rate == refresh)
				{
				...
				inner.pending_info.modes.push(Mode { dimensions: (width, height), refresh_rate: refresh, current, preferred });
...
where
	D: Dispatch<zxdg_output_manager_v1::ZxdgOutputManagerV1, UserData = Self::UserData> + OutputHandler,
{
	fn event(
		_: &mut D,
	...
	fn new_global(data: &mut D, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, interface: &str, version: u32) {
		match interface {
			"wl_output" => {
				let wl_output = data
					.registry()
					.bind_cached::<wl_output::WlOutput, _, _, _>(conn, qh, name, || (u32::min(version, 4), OutputData::new()))
			...
			"zxdg_output_manager_v1" => {
				let global = data
					.registry()
					.bind_once::<zxdg_output_manager_v1::ZxdgOutputManagerV1, _, _>(conn, qh, name, u32::min(version, 3), ())
					.expect("Failed to bind global");
