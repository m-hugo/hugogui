...
impl RegistryState {
	...
	pub fn bind_once<I, D, U>(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, version: u32, udata: U) -> Result<I, BindError>
	where
	...
	///
	/// A protocol error will be risen if the global has not yet been advertised.
	pub fn bind_cached<I, D, F, U>(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, f: F) -> Result<I, BindError>
	where
		D: Dispatch<I, UserData = U> + 'static,
		I: Proxy + 'static,
		U: Send + Sync + 'static,
		F: FnOnce() -> (u32, U),
	{
		match self.cached_globals.get(&name) {
			...
			None => {
				...
				let removed =
					self.cached_globals.insert(name, CachedGlobal { name, _version: version, interface: I::interface().name, id: global.id() });
