...
pub trait CompositorHandler: Sized {
	...
	fn scale_factor_changed(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, surface: &wl_surface::WlSurface, new_factor: i32);
	...
	fn frame(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, surface: &wl_surface::WlSurface, time: u32);
}
...
impl CompositorState {
	...
	pub fn create_surface<D>(&self, conn: &mut ConnectionHandle, qh: &QueueHandle<D>) -> Result<wl_surface::WlSurface, SurfaceError>
	where
		D: Dispatch<wl_surface::WlSurface, UserData = SurfaceData> + 'static,
	{
		let (_, compositor) = self.wl_compositor.as_ref().ok_or(SurfaceError::MissingCompositorGlobal)?;

		let surface = compositor.create_surface(conn, qh, SurfaceData { scale_factor: AtomicI32::new(1), outputs: Mutex::new(vec![]) })?;
...
where
	D: Dispatch<wl_surface::WlSurface, UserData = Self::UserData> + Dispatch<wl_output::WlOutput, UserData = OutputData> + CompositorHandler,
{
	fn event(
		state: &mut D,
		...
		let largest_factor = outputs.iter().filter_map(|output| output.data::<OutputData>().map(OutputData::scale_factor)).reduce(i32::max);
...
{
	fn event(_: &mut D, _: &wl_compositor::WlCompositor, _: wl_compositor::Event, _: &(), _: &mut ConnectionHandle, _: &QueueHandle<D>) {
...
where
	D: Dispatch<wl_compositor::WlCompositor, UserData = ()> + CompositorHandler + ProvidesRegistryState + 'static,
{
	fn new_global(state: &mut D, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, interface: &str, version: u32) {
		if interface == "wl_compositor" {
			let compositor = state
				.registry()
				.bind_once::<wl_compositor::WlCompositor, _, _>(conn, qh, name, u32::min(version, 4), ())
	...
	fn remove_global(state: &mut D, _conn: &mut ConnectionHandle, _qh: &QueueHandle<D>, name: u32) {
		if state.compositor_state().wl_compositor.as_ref().map(|(compositor_name, _)| *compositor_name == name).unwrap_or(false) {
