...
impl Window {
	...
	pub(crate) fn impl_clone(&self) -> Window {
		Window { inner: self.inner.clone(), primary: false, death_signal: self.death_signal.clone() }
	}
}
...
impl WindowInner {
	...
	pub fn show_window_menu(&self, conn: &mut ConnectionHandle, seat: &wl_seat::WlSeat, serial: u32, x: u32, y: u32) {
	...
	pub fn set_fullscreen(&self, conn: &mut ConnectionHandle, output: Option<&wl_output::WlOutput>) {
	...
	pub fn request_decoration_mode(&self, conn: &mut ConnectionHandle, mode: Option<DecorationMode>) {
		if let Some(toplevel_decoration) = &self.toplevel_decoration {
			match mode {
				Some(DecorationMode::Client) => toplevel_decoration.set_mode(conn, Mode::ClientSide),
				Some(DecorationMode::Server) => toplevel_decoration.set_mode(conn, Mode::ServerSide),
	...
	fn new_global(data: &mut D, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, interface: &str, _version: u32) {
		if "zxdg_decoration_manager_v1" == interface {
			...
			let xdg_decoration_manager = data
				.registry()
				.bind_once::<zxdg_decoration_manager_v1::ZxdgDecorationManagerV1, _, _>(conn, qh, name, DECORATION_MANAGER_VERSION, ())
	...
	fn event(
		...
		match event {
			xdg_toplevel::Event::Configure { width, height, states } => {
				...
				let new_size = if width == 0 && height == 0 { None } else { Some((width as u32, height as u32)) };
...
where
	D: Dispatch<zxdg_decoration_manager_v1::ZxdgDecorationManagerV1, UserData = Self::UserData> + WindowHandler,
{
	fn event(
		_: &mut D,
...
impl DelegateDispatchBase<zxdg_toplevel_decoration_v1::ZxdgToplevelDecorationV1> for XdgWindowState {
	type UserData = WindowData;
}

impl<D> DelegateDispatch<zxdg_toplevel_decoration_v1::ZxdgToplevelDecorationV1, D> for XdgWindowState
where
	D: Dispatch<zxdg_toplevel_decoration_v1::ZxdgToplevelDecorationV1, UserData = Self::UserData> + WindowHandler,
{
	fn event(
		_: &mut D,
