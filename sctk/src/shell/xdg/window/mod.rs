...
pub trait WindowHandler: XdgShellHandler + Sized {
	...
	fn request_close(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, window: &Window);
	...
	/// all been sent.
	fn configure(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, window: &Window, configure: WindowConfigure, serial: u32);
}
...
impl WindowBuilder {
	...
	where
		D: Dispatch<xdg_surface::XdgSurface, UserData = XdgSurfaceData<D>>
			...
			+ 'static,
	{
		let decoration_manager = window_state.xdg_decoration_manager.as_ref().map(|(_, global)| global);
		...
		let xdg_surface = shell_state.create_xdg_surface(conn, qh, surface, WindowConfigureHandler { data: data.clone() })?;
		...
		let window = Window { inner, primary: true, death_signal: Arc::new(AtomicBool::new(false)) };
...
impl Window {
	...
	pub fn show_window_menu(&self, conn: &mut ConnectionHandle, seat: &wl_seat::WlSeat, serial: u32, position: (u32, u32)) {
...
where
	D: WindowHandler,
{
	fn configure(&self, data: &mut D, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, xdg_surface: &xdg_surface::XdgSurface, serial: u32) {
