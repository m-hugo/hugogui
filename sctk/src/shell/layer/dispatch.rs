...
use super::{LayerHandler, LayerState, LayerSurface, LayerSurfaceConfigure, LayerSurfaceData, SurfaceKind};

impl<D> RegistryHandler<D> for LayerState
where
	D: Dispatch<zwlr_layer_shell_v1::ZwlrLayerShellV1, UserData = ()> + LayerHandler + ProvidesRegistryState + 'static,
{
	fn new_global(data: &mut D, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, interface: &str, version: u32) {
		if interface == "zwlr_layer_shell_v1" {
			...
			data.layer_state().wlr_layer_shell = Some(
				data.registry()
					.bind_once::<zwlr_layer_shell_v1::ZwlrLayerShellV1, _, _>(conn, qh, name, u32::min(version, 4), ())
...
where
	D: Dispatch<zwlr_layer_surface_v1::ZwlrLayerSurfaceV1, UserData = Self::UserData> + LayerHandler + 'static,
{
	fn event(
		data: &mut D,
