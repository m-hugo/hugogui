...
impl RawPool {
	...
	where
		...
		U: Send + Sync + 'static,
	{
		let buffer = self.pool.create_buffer(conn, offset, width, height, stride, format, qh, udata)?;
...
impl RawPool {
	...
	fn create_shm_fd() -> io::Result<RawFd> {
		...
		let mut mem_file_handle = format!("/smithay-client-toolkit-{}", time.duration_since(UNIX_EPOCH).unwrap().subsec_nanos());

		loop {
			let flags = fcntl::OFlag::O_CREAT | fcntl::OFlag::O_EXCL | fcntl::OFlag::O_RDWR | fcntl::OFlag::O_CLOEXEC;
			...
			match mman::shm_open(mem_file_handle.as_str(), flags, mode) {
				...
				Err(Errno::EEXIST) => {
					...
					mem_file_handle = format!("/smithay-client-toolkit-{}", time.duration_since(UNIX_EPOCH).unwrap().subsec_nanos());
	...
	fn create_memfd() -> nix::Result<RawFd> {
		...
		loop {
			...
			match memfd::memfd_create(name, flags) {
				Ok(fd) => {
					let arg = FcntlArg::F_ADD_SEALS(SealFlag::F_SEAL_SHRINK | SealFlag::F_SEAL_SEAL);
