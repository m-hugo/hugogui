...
impl ShmState {
	...
	pub fn new_simple_pool<D, U>(&self, len: usize, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, udata: U) -> Result<SimplePool, CreatePoolError>
	where
	...
	pub fn new_raw_pool<D, U>(&self, len: usize, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, udata: U) -> Result<RawPool, CreatePoolError>
	where
		D: Dispatch<wl_shm_pool::WlShmPool, UserData = U> + 'static,
		U: Send + Sync + 'static,
	{
...
{
	fn event(state: &mut D, _proxy: &wl_shm::WlShm, event: wl_shm::Event, _: &Self::UserData, _: &mut ConnectionHandle, _: &QueueHandle<D>) {
		match event {
...
{
	fn event(_: &mut D, _: &wl_shm_pool::WlShmPool, _: wl_shm_pool::Event, _: &(), _: &mut ConnectionHandle, _: &QueueHandle<D>) {
...
{
	fn new_global(state: &mut D, conn: &mut ConnectionHandle, qh: &QueueHandle<D>, name: u32, interface: &str, _: u32) {
		if interface == "wl_shm" {
			let shm = state.registry().bind_once::<wl_shm::WlShm, _, _>(conn, qh, name, 1, ()).expect("Failed to bind global");
