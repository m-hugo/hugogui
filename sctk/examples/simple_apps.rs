...
use smithay_client_toolkit::{
	...
	shell::layer::{Anchor, KeyboardInteractivity, Layer, LayerHandler, LayerState, LayerSurface, LayerSurfaceConfigure},
	...
	pub fn draw(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>) {
		if let Some(window) = self.layer.as_ref() {
			...
			// Draw to the window:
			{
				...
				let glyphs: Vec<_> = font.layout("Keyboard", scale, offset).collect();
