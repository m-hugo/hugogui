...
impl touch::TouchHandler for SimpleLayer {
	...
	fn motion(
		...
		if let Some(sp) = self.startpoint {
			...
			if difx.abs() < 10. {
				if dify <= -20. {
					Command::new("sh").args(["gestures.sh", "LeftEdgeSlideUp", &(-dify - 19.9).to_string()]).status();
					self.startpoint = Some(position);
					println!("LeftEdgeSlideUp");
					return;
				}
				if dify >= 20. {
					Command::new("sh").args(["gestures.sh", "LeftEdgeSlideDown", &(dify - 19.9).to_string()]).status();
					self.startpoint = Some(position);
					println!("LeftEdgeSlideDown");
					return;
				}
			}
			...
			if dify.abs() < 40. {
				if difx >= 80. {
					...
					if position.1 < (self.height / 3).into() {
						Command::new("sh").args(["gestures.sh", "LeftEdgePullUp"]).status();
						println!("LeftEdgePullUp");
					} else if position.1 < (2 * self.height / 3).into() {
						Command::new("sh").args(["gestures.sh", "LeftEdgePullMid"]).status();
						println!("LeftEdgePullMid");
					} else {
						Command::new("sh").args(["gestures.sh", "LeftEdgePullDown"]).status();
						println!("LeftEdgePullDown");
					}
				}
			}
		}
