...
use smithay_client_toolkit::{
	...
	delegate_compositor, delegate_keyboard, delegate_output, delegate_pointer, delegate_registry, delegate_seat, delegate_shm, delegate_xdg_shell,
	delegate_xdg_window,
...
fn main() {
	...
	let pool = simple_window
		.shm_state
		.new_raw_pool(simple_window.width as usize * simple_window.height as usize * 4, &mut conn.handle(), &qh, ())
	...
	let window = Window::builder()
		...
		.min_size((256, 256))
		.map(&mut conn.handle(), &qh, &simple_window.xdg_shell_state, &mut simple_window.xdg_window_state, surface)
...
impl WindowHandler for SimpleWindow {
	...
	fn configure(&mut self, conn: &mut ConnectionHandle, qh: &QueueHandle<Self>, _window: &Window, configure: WindowConfigure, _serial: u32) {
